import { drawGravityFieldWebGl, type GravitySurfaceBody } from "./fields-webgl-renderer.js";
...
export function normalizeFieldsDocument(schemaId: string | undefined, value: unknown): unknown {
  ...
  const documentSchema = tuple ? tuple[0] : (value as { schema?: unknown } | undefined)?.schema;
  const schema = String(schemaId || documentSchema || "");
  if (!tuple || !FIELD_SCHEMAS.has(schema)) return value;
  if (schema === "gamecult.fields.splats.v1") return normalizeSplats(tuple);
  if (schema === "gamecult.fields.gravity.v1") return normalizeGravity(tuple);
  return normalizeObjects(tuple);
}

function drawGravitySurface(
  canvas: HTMLCanvasElement,
  props: Record<string, unknown>,
  state: FieldsSurfaceDataState | undefined,
  resolveAssetUrl: (uri: string) => string,
): void {
...
function normalizeSplats(value: unknown[]): Record<string, unknown> {
...
function normalizeGravity(value: unknown[]): Record<string, unknown> {
  return {
    schema: value[0], frameId: value[1], publishedAtUtc: value[2], simulationTimeSeconds: value[3],
    runId: value[4], zoneIndex: value[5], zoneName: value[6], viewport: normalizeViewport(value[7]),
...
function normalizeObjects(value: unknown[]): Record<string, unknown> {
  return {
    schema: value[0], frameId: value[1], publishedAtUtc: value[2], simulationTimeSeconds: value[3],
...
function normalizeViewport(value: unknown): unknown {
  const row = array(value);
  return row.length ? { minX: row[0], minY: row[1], maxX: row[2], maxY: row[3] } : value;
}

function normalizeSplatLayer(value: unknown): unknown {
  const row = array(value);
  if (!row.length) return value;
  return {
...
function normalizeGravityInfluence(value: unknown): unknown {
  const row = array(value);
  if (!row.length) return value;
  return {
