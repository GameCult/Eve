...
export function buildElectronReleaseRequest({
  capabilityManifest,
  packageManifest,
  capabilityManifestPath = "",
  packageManifestPath = "",
  repository = "GameCult/EveElectron",
} = {}) {
  ...
  const artifactPath = replaceVersion(
    releaseContract.artifactPattern || `artifacts/eveelectron-release/{version}/${releaseContract.packageName}-{version}.zip`,
...
function runCli() {
  ...
  const request = buildElectronReleaseRequest({
    ...
    repository: args.repository || "GameCult/EveElectron",
  });
